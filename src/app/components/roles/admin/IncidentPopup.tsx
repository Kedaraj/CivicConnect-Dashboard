import { MapPin, Clock, Navigation, X, CheckCircle, Phone, Trash2 } from 'lucide-react';
import { motion } from 'motion/react';

export function IncidentPopup({ incident, onClose, onResolve, onDelete }: { incident: any; onClose: () => void; onResolve: (id: string) => void; onDelete?: (id: string) => void }) {
  const solved = incident.status === 'resolved' || incident.status === 'closed';
  const openNav = () => {
    const { lat, lng } = incident.location || {};
    if (lat && lng) navigator.geolocation?.getCurrentPosition(
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
      <motion.div initial={{ y: 200 }} animate={{ y: 0 }} exit={{ y: 200 }} className="bg-white rounded-t-3xl w-full max-w-lg max-h-[85vh] overflow-auto">
        ...
        <div className="p-4 space-y-3">
          {solved ? (
            <div className="rounded-xl p-3 bg-green-50 border border-green-200 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-green-600" /><span className="text-green-700 font-bold text-sm">✅ RESOLVED</span></div>
          ) : (
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-2">
            ...
            {incident.reportedBy?.name && <p className="text-xs text-gray-400">By: {incident.reportedBy.name} ({incident.reportedBy.role})</p>}
          </div>
          {incident.location?.lat && <button onClick={openNav} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2"><Navigation className="w-5 h-5" />Navigate</button>}
          <div className="grid grid-cols-2 gap-2">
            {!solved && <button onClick={() => { onResolve(incident._id); onClose(); }} className="py-2.5 bg-green-500 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-1"><CheckCircle className="w-4 h-4" />Resolve</button>}
            {onDelete && <button onClick={() => { onDelete(incident._id); onClose(); }} className="py-2.5 bg-red-50 text-red-600 rounded-xl text-sm font-medium flex items-center justify-center gap-1"><Trash2 className="w-4 h-4" />Delete</button>}
          </div>
